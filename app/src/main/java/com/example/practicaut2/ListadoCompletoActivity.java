...
public class ListadoCompletoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor = myDB.rawQuery("select * from alumno order by grupo",null);
    ...
    public void pulsacionBtnBuscar(View v) {
        Spinner selectorSexos = (Spinner)findViewById(R.id.spinnerSexos);
        Spinner selectorGrupos = (Spinner)findViewById(R.id.spinnerGrupos);
        ArrayList<Alumno> lista = new ArrayList<>();
        String seleccionSexo =(String)  selectorSexos.getSelectedItem();
        String seleccionGrupo =(String)  selectorGrupos.getSelectedItem();
        SQLiteDatabase myDB = openOrCreateDatabase(getResources().getString(R.string.db), MODE_PRIVATE, null);
        Cursor cursor = null;
        if(seleccionGrupo.equals("Todos") && seleccionSexo.equals("Todos")){
            cursor = myDB.rawQuery("select * from alumno order by grupo",null);
